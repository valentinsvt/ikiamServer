...
<div class="fieldcontain ${hasErrors(bean: fotoInstance, field: 'entry', 'error')} required">
	...
	<g:select id="entry" name="entry.id" from="${ikiam.Entry.list()}" optionKey="id" required="" value="${fotoInstance?.entry?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: fotoInstance, field: 'especie', 'error')} required">
	...
	<g:select id="especie" name="especie.id" from="${ikiam.Especie.list()}" optionKey="id" required="" value="${fotoInstance?.especie?.id}" class="many-to-one"/>
</div>
